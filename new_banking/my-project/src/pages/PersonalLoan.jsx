...
const EligibilityChecker = ({ onCheck }) => {
  ...
  const calculateEligibleAmount = (data) => {
    const baseAmount = parseInt(data.monthlyIncome) * 12;
    const multiplier = data.creditScore > 750 ? 1.5 : 1.2;
    return Math.min(baseAmount * multiplier, 2000000);
  };
  ...
  const features = [
    {
      ...
      description: "Get your loan amount credited within 24 hours of approval",
      highlight: "24 Hours",
    ...
    },
    {
      ...
      description: "Enjoy interest rates starting from 8.5% p.a.",
      highlight: "8.5% p.a.",
...
const LoanApplicationModal = ({ isOpen, onClose, onSubmit, applicationData, onInputChange, onFileUpload, applicationStatus: initialStatus, verificationProgress }) => {
  ...
  const [verificationDetails, setVerificationDetails] = useState({
    ...
    employment: { status: 'pending', message: 'Verifying employment details...' },
    bank: { status: 'pending', message: 'Verifying bank statements...' }
  });
  ...
  const steps = [
    ...
    { id: 4, title: 'Documents', icon: <FaFileAlt /> },
    ...
    switch (step) {
      case 1:
        if (!applicationData.amount || applicationData.amount < 50000) {
          errors.amount = 'Minimum loan amount is ₹50,000';
        }
        if (!applicationData.purpose) {
          errors.purpose = 'Please select a loan purpose';
        }
        if (!applicationData.tenure || applicationData.tenure < 12 || applicationData.tenure > 60) {
          errors.tenure = 'Tenure must be between 12 and 60 months';
        }
        break;
      case 2:
        if (!applicationData.panNumber || !/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/.test(applicationData.panNumber)) {
          errors.panNumber = 'Please enter a valid PAN number';
        }
        if (!applicationData.aadharNumber || !/^\d{12}$/.test(applicationData.aadharNumber)) {
          errors.aadharNumber = 'Please enter a valid Aadhaar number';
        }
      ...
      case 3:
        ...
        if (!applicationData.monthlyIncome || applicationData.monthlyIncome < 15000) {
          errors.monthlyIncome = 'Minimum monthly income should be ₹15,000';
        }
        if (applicationData.employmentType === 'salaried' && !applicationData.companyName) {
          errors.companyName = 'Please enter company name';
        }
        break;
      case 4:
        if (!uploadedFiles.identity?.length) {
          errors.identity = 'Please upload identity proof';
        }
        if (!uploadedFiles.income?.length) {
          errors.income = 'Please upload income proof';
        }
        if (!uploadedFiles.bank?.length) {
          errors.bank = 'Please upload bank statements';
        }
        break;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const verificationSteps = ['identity', 'income', 'credit', 'employment', 'bank'];
      ...
      const principal = parseFloat(applicationData.amount);
      const rate = 10.5 / 12 / 100; // 10.5% p.a.
      const time = parseInt(applicationData.tenure);
      const emi = principal * rate * Math.pow(1 + rate, time) / (Math.pow(1 + rate, time) - 1);
      
      const finalLoanDetails = {
        loanId: `PL${Math.floor(Math.random() * 1000000)}`,
        amount: principal,
        tenure: time,
        emi: Math.round(emi),
        disbursalDate: new Date(Date.now() + 24 * 60 * 60 * 1000).toLocaleDateString(),
        interestRate: '10.5% p.a.',
        processingFee: Math.round(principal * 0.02), // 2% processing fee
        totalAmount: Math.round(emi * time)
      };
  ...
  const renderCongratsModal = () => {
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-xl shadow-lg max-w-lg w-full p-6 sm:p-8 text-center">
          <div className="w-16 h-16 sm:w-20 sm:h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6">
            <FaCheck className="w-8 h-8 sm:w-10 sm:h-10 text-green-600" />
          </div>
          <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-4">Congratulations!</h3>
          <p className="text-gray-600 mb-6">Your personal loan application has been approved successfully.</p>
          
          <div className="bg-gray-50 rounded-lg p-4 sm:p-6 mb-6">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-gray-500">Loan ID</p>
                <p className="font-semibold text-gray-900 truncate">{loanDetails.loanId}</p>
              </div>
              <div>
              ...
              <div>
                <p className="text-sm text-gray-500">Monthly EMI</p>
                <p className="font-semibold text-gray-900">₹{loanDetails.emi.toLocaleString()}</p>
              </div>
              <div>
          ...
          <div className="space-y-3">
            <button
              ...
              className="w-full px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors duration-200 text-sm sm:text-base"
            >
              Done
            </button>
            <button
              ...
              className="w-full px-6 py-3 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors duration-200 text-sm sm:text-base"
            >
              Download Details
            </button>
          </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-8 pr-4 py-2 border ${formErrors.amount ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500`}
            ...
            <div>
              ...
              <select
                ...
                onChange={onInputChange}
                className={`w-full px-4 py-2 border ${formErrors.purpose ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500`}
                required
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={onInputChange}
                className={`w-full px-4 py-2 border ${formErrors.tenure ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500`}
                placeholder="Enter tenure (12-60 months)"
                min="12"
                max="60"
                required
              />
              {formErrors.tenure && (
                <p className="mt-1 text-sm text-red-500">{formErrors.tenure}</p>
              )}
            </div>
          </div>
        );
          ...
          <div className="space-y-6">
            <div>
              ...
              <input
                ...
                onChange={onInputChange}
                className={`w-full px-4 py-2 border ${formErrors.panNumber ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500`}
            ...
            <div>
              ...
              <input
                ...
                onChange={onInputChange}
                className={`w-full px-4 py-2 border ${formErrors.aadharNumber ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500`}
                placeholder="Enter Aadhaar number"
                required
              />
              {formErrors.aadharNumber && (
                <p className="mt-1 text-sm text-red-500">{formErrors.aadharNumber}</p>
              )}
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={onInputChange}
                className={`w-full px-4 py-2 border ${formErrors.dateOfBirth ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500`}
                required
              />
          ...
          <div className="space-y-6">
            <div>
              ...
              <select
                ...
                onChange={onInputChange}
                className={`w-full px-4 py-2 border ${formErrors.employmentType ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500`}
                required
              >
              ...
              {formErrors.employmentType && (
                <p className="mt-1 text-sm text-red-500">{formErrors.employmentType}</p>
              )}
            </div>
            ...
            {applicationData.employmentType === 'salaried' && (
              <div>
                ...
                <input
                  ...
                  onChange={onInputChange}
                  className={`w-full px-4 py-2 border ${formErrors.companyName ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500`}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={onInputChange}
                  className={`w-full pl-8 pr-4 py-2 border ${formErrors.monthlyIncome ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500`}
                  placeholder="Enter monthly income (min. ₹15,000)"
                  required
                />
                {formErrors.monthlyIncome && (
                  <p className="mt-1 text-sm text-red-500">{formErrors.monthlyIncome}</p>
                )}
              </div>
            </div>
          </div>
              ...
              <div>
                ...
                <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg">
                  <div className="space-y-1 text-center">
                    ...
                    {formErrors.identity && (
                      <p className="mt-1 text-sm text-red-500">{formErrors.identity}</p>
                    )}
              ...
              <div>
                ...
                <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg">
                  <div className="space-y-1 text-center">
                    ...
                    {formErrors.bank && (
                      <p className="mt-1 text-sm text-red-500">{formErrors.bank}</p>
                    )}
                    {uploadedFiles.bank && (
                      <div className="mt-2">
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="bg-indigo-50 p-4 rounded-lg">
              <h4 className="text-lg font-medium text-indigo-900 mb-2">Important Notes</h4>
              <ul className="list-disc list-inside space-y-2 text-sm text-indigo-700">
                <li>Processing fee of 2% will be charged on loan amount</li>
                <li>Interest rate starts from 10.5% p.a.</li>
                <li>Pre-closure charges apply after 6 months</li>
                <li>Loan disbursal within 24 hours of approval</li>
              </ul>
            </div>
          </div>
        );
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-xl shadow-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="sticky top-0 bg-white z-10 flex items-center justify-between p-4 sm:p-6 border-b border-gray-200">
            <h3 className="text-lg sm:text-xl font-semibold text-gray-900">Personal Loan Application</h3>
            <button
              onClick={onClose}
          ...
          {/* Progress Steps */}
          <div className="sticky top-[72px] sm:top-[88px] bg-white z-10 px-4 sm:px-8 py-4 border-b border-gray-200">
                    ...
                    <div key={key} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                      <div className="flex items-center space-x-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <span className="text-sm font-medium text-gray-900 capitalize block truncate">{key}</span>
              ...
              <>
                ...
                <div className="mt-8 flex justify-between max-w-2xl mx-auto">
                  ...
                  <button
                    ...
                    className={`px-4 sm:px-6 py-2 sm:py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm sm:text-base ${
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-24">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-indigo-600 via-blue-600 to-indigo-700 mb-12">
          ...
          <div className="absolute top-0 right-0 w-1/3 h-full">
            ...
            <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1579621970563-ebec7560ff3e?w=800&h=600&fit=crop')] bg-cover bg-center mix-blend-overlay opacity-20"></div>
          </div>
              ...
              <div className="max-w-2xl">
                <div className="inline-block px-6 py-2.5 bg-white/10 rounded-full backdrop-blur-sm mb-8 border border-white/20">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm font-semibold text-white tracking-wide">Personal Loans up to ₹25 Lakhs</span>
                  </div>
                </div>
                ...
                <p className="text-xl md:text-2xl text-indigo-100 mb-10 leading-relaxed max-w-xl">
                  ...
                  <span className="font-semibold text-white">Get your loan approved in 24 hours!</span>
                </p>
        ...
        <div className="mt-8 bg-gradient-to-r from-indigo-600 to-blue-600 rounded-xl p-8 text-center transform hover:scale-[1.02] transition-transform duration-300">
          ...
          <p className="text-indigo-100 mb-6">Apply now and get instant approval for your personal loan</p>
          <button 
            onClick={handleApplyNow}
